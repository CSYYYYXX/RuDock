...
pub fn schema() -> Value {
    serde_json::json!({
        ...
        "methods": [
            ...
            {"name": "plugin.market.sources", "params": {}, "returns": {"type": "object"}, "status": "M5"},
            {"name": "plugin.market.source.add", "params": {"index": "path|http(s) URL"}, "returns": {"type": "object"}, "status": "M5"},
            {"name": "plugin.market.source.remove", "params": {"index": "path|http(s) URL"}, "returns": {"type": "object"}, "status": "M5"},
...
mod tests {
    ...
    fn schema_exposes_daemon_stop() {
        ...
        let settings = methods
            ...
            .unwrap();
        assert_eq!(settings["params"]["onboarding_complete"], "boolean?");
        assert_eq!(settings["params"]["desktop_widgets"], "string[]?");
    }
}
