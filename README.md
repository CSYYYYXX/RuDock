...
## 当前状态（M5 起步：插件 Skill 已接入 Agent 能力面）
...
- **Everything**：本机未安装，daemon 启动时检测并打日志（`Everything present: false`），文件搜索走本地降级；真 IPC 集成排到 M1.5。
...
- `wb daemon stop`、Everything 真 IPC、面板真前端（WebMessage 通道）：按里程碑 M1.5–M2 排期。
