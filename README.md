...
后台服务会预先建立应用索引，并管理面板、托盘图标和 Win 键监听。首次启动面板会显示一次简短引导：可以选择界面语言，并决定是否接管 Win 键；之后仍可在设置中修改。首次启动后，也可以在设置中开启 **接管 Win 键** 和 **开机启动**，或使用 CLI：
...
RuDock 不会在后台自动下载或替换程序。可以在设置中点击 **检查更新**，或使用 CLI 手动查询 GitHub 最新稳定版：

```powershell
.\wb.exe update check
```

如果有新版本，结果会给出发布页地址；升级前建议先创建备份。
