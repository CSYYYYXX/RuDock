...
- 2026-08-23 全量测试基线：wb-core 12 + wb-daemon 21 + wb-plugin-sdk 12 + wb-plugin-host 6 + wb-cli 20 + wb-mcp 14 + wb-panel 3，共 88 个单测；workspace test、严格 Clippy（非 wb-panel）和 release build 通过（wb-panel 仍有原有 11 条 warning）。MCP 配置隔离测试覆盖 JSON/TOML 保留、幂等、冲突、强制替换、原子写入和卸载，真实 CLI `--file` 也完成两种格式的 install→status→uninstall。升级检查覆盖 SemVer 标签、无正式 Release、发布链接校验；本机应用索引 341 项、后台文件索引实测 43,927 项；应用列表/搜索热路径无 PowerShell 子进程，Everything 在线真实 IPC 与离线降级、MCP 动态目录通知、read-only 阻断、ask 无能力拒绝、双向 elicitation 接受后真实写入/清理均通过。组件调整大小的 pointer→RPC→settings 持久化链路已在真实 WebView2 宿主冒烟，测试布局随后恢复；日语组件页与英文应用页实机检查无 JS 错误。桌面组件既有无隐私视觉回归截图为 `docs-assets/desktop-widgets.png`。本轮源码提交 `18a1999`、`434edd2`，便携 ZIP SHA-256 更新为 `fa13e63297757abe5f6e2ca0277096c37506cd16b36ff51a98f0a0b5c46ef5ab`。

## 6. 已知瑕疵 / 未验证声明

### 2026-08-23 本轮修复

- 回归修复（应用、设置、插件）：应用页恢复等待 `apps.list` 后再渲染，真实索引 301 项实测显示 300 个有效应用；系统归类覆盖 `Get-StartApps` 的 Windows 包、System32 管理工具 GUID AppID，以及开始菜单 `Administrative Tools` / `System Tools` 快捷方式，系统项统一进入系统应用文件夹，第三方 UWP 和普通桌面应用留在外面。设置窗口改为标准不透明可缩放窗口，并隔离卡片 `SetWindowRgn`；秒表、剪贴板洞察 iframe 与插件自身补充透明画布和深浅色主题。应用普通/编辑、设置中心、两个插件均完成真实 WebView2/无头渲染验收。
- 桌面常驻组件：忽略 WebView2 恢复阶段的空矩形，加入分段重试和低频 z-order 修复；桌面层改用 `HWND_NOTOPMOST`，避免 Win+D/Explorer 重建后组件延迟或消失；桌面模式不再对卡片矩形重复启用原生 blur，减少世界时钟/系统状态一行的黑块闪烁。
- 应用页：修复编辑模式中局部变量遮蔽翻译函数导致的渲染异常；系统应用自动收纳为系统应用文件夹，卸载器/安装器/维护工具类条目默认过滤；新增导入程序、隐藏应用独立恢复菜单和本地用户配置。
- 组件：新增收纳组件（面板/桌面均可常驻），本地保存最多 27 个路径并以九宫格展示；组件缩放 pointermove 改为 `requestAnimationFrame` 合帧，降低布局抖动；搜索结果最大高度改为视口约束，避免遮挡输入框或溢出屏幕；增加跟随系统白天/黑夜的 CSS 色板。
- 设置：托盘右键新增“设置”，启动独立 `WBSettingsWindow`（`wb-panel.exe --wv2 --settings`），不再依赖面板内弹层；daemon 停止时会一并关闭设置窗口。
